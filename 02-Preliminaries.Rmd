...
```{r s1, message=FALSE, warning=FALSE}
library(datasets)
...
```{r s2, message=FALSE, warning=FALSE}
head(airquality)
str(airquality)
summary(airquality)
```
...
```{r s4, message=FALSE, warning=FALSE}
library(foreign)
library(plm)
...
```{r s5, message=FALSE, warning=FALSE}
library(ggplot2)
head(mpg, 5)
tail(mpg, 5)
```
...
```{r s77, message=FALSE, warning=FALSE}
...
```{r s8, message=FALSE, warning=FALSE}
...
```{r s9, message=FALSE, warning=FALSE}
...
```{r s10, message=FALSE, warning=FALSE}
...
```{r s14, message=FALSE, warning=FALSE}
str(cars)
summary(cars)
```
...
```{r s15, message=FALSE, warning=FALSE}
...
```{r s16, message=FALSE, warning=FALSE}
...
```{r s17, message=FALSE, warning=FALSE}
x <- cars$speed
y <- cars$dist
...
```{r s18, message=FALSE, warning=FALSE}
model <- lm(dist ~ speed, data = cars)
model
```
...
```{r s19, message=FALSE, warning=FALSE}
x <- cars$speed
y <- cars$dist
...
```{r s20, message=FALSE, warning=FALSE}
head(mtcars)
str(mtcars)
```
...
```{r s21, message=FALSE, warning=FALSE}
...
```{r stat26, message=FALSE, warning=FALSE}
...
```{r s27, message=FALSE, warning=FALSE}
...
```{r s28, message=FALSE, warning=FALSE}
#mean
mean(x)
#sd
...
```{r s32, message=FALSE, warning=FALSE}
library(broom)        
library(faux)      
...
plot(euler(c("Y" = ss_y - ss_both_y_x1,
             ...
             "Y&X1" = ss_both_y_x1)),
     quantities = c("A", "B", "D"),
     fills = list(fill = c("pink", "lightblue")))
```
...
plot(euler(all_pieces),
     ...
     fills = list(fill = c("pink", "lightblue", "lightgreen")))
```
...
```{r s36, echo=TRUE, warning=FALSE, message=FALSE}
...
```{r s41, warning=FALSE, message=FALSE}
# Simple OLS
...
```{r s42, warning=FALSE, message=FALSE}
set.seed(1)
...
```{r s43, warning=FALSE, message=FALSE}
# Linear model
model1 <- lm(y ~ X)
...
```{r s44, warning=FALSE, message=FALSE}
...
```{r s45, warning=FALSE, message=FALSE}
...
```{r s46, warning=FALSE, message=FALSE}
table <- matrix(NA, 2, 3)
