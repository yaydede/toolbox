...
```{r tsnn1, echo=FALSE, out.width = '85%', out.height='90%'}
knitr::include_graphics("png/rnn2.png")
```  
...
```{r tsnn3, message=FALSE, warning=FALSE}
library(tsibble)
library(fpp3)
...
```{r tsnn14, warning=FALSE, message=FALSE, cache=TRUE}
library(keras)
...
```{r tsnn17, warning=FALSE, message=FALSE, cache=TRUE}
model %>% fit(
  X[[1]][train, ,],
  y[[1]][train],
  batch_size = 12,
...
```{r tsnn19, echo=FALSE, out.width = '75%', out.height='80%'}
...
```{r tsnn20, warning=FALSE, message=FALSE, cache=TRUE}
model = keras_model_sequential() %>%
...
```{r tsnn21, warning=FALSE, message=FALSE, cache=TRUE}
model <- keras_model_sequential() %>%
  layer_lstm(units = 24,
             input_shape = c(7, 6),
             activation = "tanh") %>%
