...
```{r hy1, message=FALSE, warning = FALSE}
...
```{r hy2, message=FALSE, warning = FALSE}
...
```{r hy3, message=FALSE, warning = FALSE}
...
```{r hy4, echo=FALSE, out.width = '130%', out.height='130%'}
...
```{r hy5, message=FALSE, warning = FALSE, cache=TRUE}
...
```{r hy6, message=FALSE, warning = FALSE, cache=TRUE}
...
```{r hy7, echo=FALSE, out.width = '140%', out.height='140%'}
knitr::include_graphics("png/grid.png")
```  

Let's do it:  

```{r hy8, warning=FALSE, message=FALSE, cache=TRUE}
# Using the same data
n = 1000
set.seed(1)
x <- sort(runif(n) * 2 * pi)
...
```{r hy9, warning=FALSE, message=FALSE, cache=TRUE}
#Using the same data
n = 1000
set.seed(1)
x <- sort(runif(n) * 2 * pi)
...
```{r hy10, warning=FALSE, message=FALSE}
...
```{r hy12, warning=FALSE, message=FALSE, cache=TRUE}
#Using the same data
n = 1000
set.seed(1)
x <- sort(runif(n) * 2 * pi)
...
```{r hy13, warning=FALSE, message=FALSE}
plot(RMSPE_test, col = "red")
abline(a = mean(RMSPE_test),
       b = 0,
       col = "green",
...
```{r hy14, warning=FALSE, message=FALSE, cache=TRUE}
#Using the same data
n = 1000
set.seed(1)
x <- sort(runif(n) * 2 * pi)
...
```{r hy15, warning=FALSE, message=FALSE}
plot(RMSPE_test, col = "red")
abline(a = mean(RMSPE_test),
       b = 0,
       col = "green",
